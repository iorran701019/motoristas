...
export default function Horarios() {
  ...
  const carregar = async () => {
    ...
    const [{ data: hor }, { data: al }, { data: pr }] = await Promise.all([
      supabase.from('horarios_atendimento').select('*, alunos(nome), professores_aee(nome)').eq('ativo', true).order('dia_semana'),
  ...
  const salvar = async (e: any) => {
    e.preventDefault()
    setSalvando(true)
    await supabase.from('horarios_atendimento').insert([form])
    setForm({ aluno_id: '', professor_id: '', dia_semana: '', hora_inicio: '', hora_fim: '', local: '', tipo_atendimento: '' })
          ...
          <form onSubmit={salvar} className="bg-white rounded-xl border border-gray-200 p-6 mb-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <select value={form.aluno_id} onChange={(e) => setForm({ ...form, aluno_id: e.target.value })} required className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
              ...
              <div>
                ...
                <select value={form.professor_id} onChange={(e) => setForm({ ...form, professor_id: e.target.value })} required className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                  <option value="">Selecione...</option>
              ...
              <div>
                ...
                <select value={form.dia_semana} onChange={(e) => setForm({ ...form, dia_semana: e.target.value })} required className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                  <option value="">Selecione...</option>
              ...
              <div>
                ...
                <select value={form.tipo_atendimento} onChange={(e) => setForm({ ...form, tipo_atendimento: e.target.value })} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                  <option value="">Selecione...</option>
              ...
              <div>
                ...
                <input type="time" value={form.hora_inicio} onChange={(e) => setForm({ ...form, hora_inicio: e.target.value })} required className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Hora fim *</label>
                <input type="time" value={form.hora_fim} onChange={(e) => setForm({ ...form, hora_fim: e.target.value })} required className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
              </div>
              <div className="sm:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-1">Local</label>
                <input value={form.local} onChange={(e) => setForm({ ...form, local: e.target.value })} placeholder="Ex: Sala de Recursos Multifuncionais" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-blue-400" />
              </div>
            </div>
                      ...
                      {porDia[dia]
                        ...
                        .map((h: any) => (
                          <tr key={h.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3 text-gray-500">{h.professores_aee?.nome}</td>
                            <td className="px-4 py-3">
                              {h.tipo_atendimento && (
                                <span className="bg-blue-50 text-blue-700 px-2 py-0.5 rounded-full text-xs">{h.tipo_atendimento}</span>
                              )}
                            </td>
