...
export default function EditarAluno() {
  ...
  async function carregarAluno() {
    ...
    setForm({
      ...
      turno: data.turno || '',
      data_avaliacao_cemae: data.data_avaliacao_cemae || '',
      status_cemae: data.status_cemae || '',
      prioridade_cuidador: data.prioridade_cuidador || '',
      judicializacao: data.judicializacao || false,
      obs_judicializacao: data.obs_judicializacao || '',
    })
    setLoading(false)
  }
  ...
  async function handleSubmit(e: any) {
    ...
    setSalvando(true)
    setErro('')
    const { error } = await supabase.from('alunos').update(form).eq('id', id)
    if (error) {
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={() => router.back()} className="px-4 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50 text-gray-700">
              Cancelar
            </button>
